...
| TAS/wind source eligibility | Validation-only known-wind model upper bound and truth-free two-state causal information oracle; three-direction bootstrap and source-time/quality negative controls | **Not closed:** a 480-case multi-seed confirmation retained one qualified unflagged vertical-wind control. A residual-pattern monitor passed its `224/224` opened synthetic development matrix, but the fresh v4 sealed holdout failed `889/896` cases (false/early/late latches). It is rejected as a runtime source supervisor and is not a physical source/regime qualification. No 17-state branch, public TAS API, physical air-data source, or degraded-navigation claim may proceed until a separately justified source/regime envelope passes physical evidence. |
