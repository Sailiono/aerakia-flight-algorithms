...
| TAS/wind source eligibility | Validation-only known-wind model upper bound and truth-free two-state causal information oracle; three-direction bootstrap and source-time/quality negative controls | **Not closed:** a 480-case multi-seed confirmation retained one qualified unflagged vertical-wind control. A residual-pattern monitor now has a `224/224` opened synthetic development matrix; an exposed one-seed v3 smoke is invalidated and a fresh v4 synthetic holdout is frozen. It is not a physical source/regime qualification and cannot remove the block. No 17-state branch, public TAS API, physical air-data source, or degraded-navigation claim may proceed until a separately justified source/regime envelope passes physical evidence. |
