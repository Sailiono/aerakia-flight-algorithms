...
| TAS/wind source eligibility | Validation-only known-wind model upper bound and truth-free two-state causal information oracle; three-direction bootstrap and source-time/quality negative controls | Closed only as a 15-case / 736-observation host prerequisite for a future wind experiment. No 17-state branch, public TAS API, physical air-data source, or degraded-navigation claim exists yet. |
