...
| Bias convergence | Gyroscope and accelerometer bias error, convergence time, and steady-state uncertainty are reported separately; batch reference-bias correction is not counted as online convergence | The G0 multi-pose static-calibration candidate passed its hash-pinned, clean-tree sealed `1,096`-trial boundary holdout: `614` baseline fail-to-pass improvements and `0` regressions. This closes a synthetic cold-start seed claim only; physical IMU, thermal, vibration, and selector characterization remain open. See [sealed result](multipose-static-calibration-sealed-holdout.md). |
