...
| Public dataset runner | Dataset manifest records source, hash, frame transform, time offset, command, code commit, and output summary; selected EuRoC runs reproduce with one command | Implemented for 5 EuRoC tracks and 135,558 replayed IMU samples; keep the reviewed baseline gate passing |
