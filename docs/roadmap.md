...
| Airspeed/barometer degraded navigation | Paired IMU-only, raw barometer, supervised barometer, hot-shadow failover, known-wind TAS, estimated-wind TAS, and combined arms cover fixed-wing/VTOL regimes and 5-120 s GNSS outages | Barometer: the v1 `600/600` synthetic matrix is complete and the 100/200/400 Hz complete shadow-image handoff contract passes. TAS/wind: a 15-case / 736-observation validation-only source/observability prerequisite now passes, proving required fail-closed conditions before a branch exists. Persistent datum bias, source reset, physical pressure and air-data calibration, private scheduling/controller handling, a 17-state wind branch, and sealed physical data remain open. See [barometer study](barometer-supervision-study.md) and [TAS/wind prerequisite](airspeed-wind-observability.md). |
