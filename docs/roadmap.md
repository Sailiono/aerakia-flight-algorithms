...
| Bias convergence | Gyroscope and accelerometer bias error, convergence time, and steady-state uncertainty are reported separately; batch reference-bias correction is not counted as online convergence | The G0 multi-pose static-calibration candidate passes `548/548` causal-IMU opened-development trials versus `246/548` one-pose baseline (`302` improvements, `0` regressions). A new hash-pinned, clean-tree sealed protocol reserves eight unused seeds across the full 137-case boundary; it must run once before any candidate promotion. This remains a synthetic cold-start result only: thermal and physical characterization remain open. |
...
| Airspeed/barometer degraded navigation | Paired IMU-only, raw barometer, supervised barometer, hot-shadow failover, known-wind TAS, estimated-wind TAS, and combined arms cover fixed-wing/VTOL regimes and 5-120 s GNSS outages | Barometer: the v1 `600/600` synthetic matrix is complete, the 100/200/400 Hz complete shadow-image handoff contract passes, and the public supervised transaction API passes 17/17 CTest targets. TAS/wind: the 15-case base source/observability contract passes, but its 480-case multi-seed confirmation has one unflagged vertical-wind false qualification, so the source policy is explicitly **not closed**. A separate residual-pattern v2 development matrix passes `224/224`; the exposed one-seed v3 smoke is invalidated; the fresh v4 sealed holdout was executed once and failed `889/896` cases, including false/early/late latches. The monitor is rejected as a runtime source supervisor. Neither result establishes physical source validity. Persistent datum bias, source reset, physical pressure and air-data calibration, private scheduling/controller handling, a 17-state wind branch, and sealed physical data remain open. See [barometer study](barometer-supervision-study.md), [TAS/wind multi-seed confirmation](airspeed-wind-multiseed-confirmation.md), and [residual-pattern study](airspeed-wind-mismatch-monitor.md). |
