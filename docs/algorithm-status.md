...
| TAS/wind observability prerequisite | Offline known-wind model/Jacobian upper bound plus a truth-free causal two-state horizontal-wind information oracle, with source-time/quality contract and explicit negative controls | The 15/15 deterministic / 736-observation base matrix passes, but a 480-case / 23,552-observation frozen confirmation reopens vertical wind: one of 32 unflagged vertical-wind replications ended qualified. V4 is permanently retained as a failed `889/896` sealed holdout. A separate v5 opened consecutive-evidence diagnostic ran 640 replications / 87,744 observations: nominal, high-noise nominal, and single-impulse controls were clean, but nine genuine onset-contamination failures remained; 62 additional raw failures exposed gap-counter instrumentation rather than latches. V5 is rejected, the monitor remains unauthorized as a runtime supervisor, and no wind state/API or degraded-navigation claim exists; see [TAS/wind multi-seed confirmation](airspeed-wind-multiseed-confirmation.md) and [residual-pattern study](airspeed-wind-mismatch-monitor.md). |
