...
| TAS/wind observability prerequisite | Offline known-wind model/Jacobian upper bound plus a truth-free causal two-state horizontal-wind information oracle, with source-time/quality contract and explicit negative controls | The 15/15 deterministic / 736-observation base matrix passes, but a 480-case / 23,552-observation frozen confirmation reopens vertical wind: one of 32 unflagged vertical-wind replications ended qualified. The other 31 were non-qualified/latching outcomes, but the false qualifier blocks promotion. No wind state/API or degraded-navigation claim exists; see [TAS/wind multi-seed confirmation](airspeed-wind-multiseed-confirmation.md). |
