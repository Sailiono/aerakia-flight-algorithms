...
| TAS/wind observability prerequisite | Offline known-wind model/Jacobian upper bound plus a truth-free causal two-state horizontal-wind information oracle, with source-time/quality contract and explicit negative controls | The 15/15 deterministic / 736-observation base matrix passes, but a 480-case / 23,552-observation frozen confirmation reopens vertical wind: one of 32 unflagged vertical-wind replications ended qualified. A residual-pattern monitor's v1 review failure is retained; its v2 opened development matrix passes `224/224` over 30,528 source observations with explicit nominal/impulse/gap coverage. An intended v3 smoke was invalidated after one seed was exposed; the fresh v4 sealed holdout then failed `889/896` cases, including false/early/late latches. The monitor is rejected as a runtime supervisor and neither monitor changes the false-qualification block: no wind state/API or degraded-navigation claim exists; see [TAS/wind multi-seed confirmation](airspeed-wind-multiseed-confirmation.md) and [residual-pattern study](airspeed-wind-mismatch-monitor.md). |
