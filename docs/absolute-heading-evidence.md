...
The repository therefore has independent absolute-yaw *reference* data and a physical heading-input
track, but it does not yet have a completed Class A end-to-end result. This distinction is why the
current maturity table says independent physical absolute heading is unproved.
...
1. INSANE `indoor_1` and `transition_1` are the first candidates. They are small enough for an
   immediate intake (about 53.6 MB together), provide independent OptiTrack full pose/yaw, and can
   reuse the existing INSANE frame/time tooling. Before promotion, the raw archive must confirm a
   usable magnetometer or other physical heading observation, its calibration, and the additional
   non-commercial dataset terms. If that input exists and is source-independent from OptiTrack,
   these tracks can become Class A.
