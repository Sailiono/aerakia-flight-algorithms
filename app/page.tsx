...
function actualPoints(rows: Row[], axis: "roll" | "pitch" | "yaw", start: number, end: number, mode: PlotMode) {
  const selected = rows.filter((row) => row.t >= start && row.t <= end);
  const stride = Math.max(1, Math.floor(selected.length / 1600));
  return selected
    .filter((_, i) => i % stride === 0 || i === selected.length - 1)
    .map((row) => [row.t, mode === "geodesic" ? row.geo : mode === "error"
      ? row[axis].map((value, index) => index === 0 ? 0 : wrapAngleDeg(value - row[axis][0]))
      : row[axis]]);
}
function path(values: number[][], index: number, min: number, max: number, start: number, end: number, breakOnJump: boolean) {
...
export default function Home() {
  ...
  const range = useMemo(() => {
    ...
    const end = start + span; const points = actualPoints(curve.rows, axis, start, end, plotMode);
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <aside className="panel">
        ...
        <p className="label below">绘制方式</p><div className="seg"><button className={plotMode === "geodesic" ? "selected" : ""} onClick={() => setPlotMode("geodesic")}>姿态几何误差</button><button className={plotMode === "error" ? "selected" : ""} onClick={() => setPlotMode("error")}>最短角差</button><button className={plotMode === "angle" ? "selected" : ""} onClick={() => setPlotMode("angle")}>原始 Euler 角</button></div>
      ...
      <section className="viewer">
        <div className="viewer-head"><div><p className="label">真实曲线查看器</p><h2>{curve.dataset} · {plotMode === "geodesic" ? "姿态几何误差" : `${axis} · ${plotMode === "error" ? "最短角差" : "原始 Euler 角"}`}</h2></div><div className="legend">{names.map((name, i) => <span key={name}><i style={{ background: colors[i] }} />{plotMode !== "angle" && i === 0 ? "0° 基线" : name}</span>)}</div></div>
        <div className="chart">
          <svg viewBox="0 0 1000 380" preserveAspectRatio="none" onWheel={onWheel} onPointerMove={onMove} onPointerDown={(event) => { drag.current = { x: event.clientX, center }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerUp={() => { drag.current = null; }} onPointerLeave={() => { drag.current = null; setHover(null); }}>
            ...
            {colors.map((color, index) => <path key={color} d={path(range.points, index, range.min, range.max, range.start, range.end, plotMode !== "geodesic")} fill="none" stroke={color} strokeWidth={index === 0 ? 1.4 : 2} vectorEffect="non-scaling-stroke" />)}
    ...
    <section className="footnote"><b>数据口径</b><span>默认姿态几何误差来自每个真实采样点的四元数 geodesic angle（真值基线为 0°）。最短角差和原始 Euler 角是诊断视图；其中 179° 与 -179° 的误差按 2° 计算，原始 Euler 角保留 CSV 的 [-180°, 180°] 表示并在分支跳变处断线，不制造跨越 360° 的假曲线。为保证渲染响应速度，全程视图仅绘制窗口内的原始点抽样；放大后自动使用更高密度的真实点，不进行曲线拟合或伪造插值。</span></section>
  </main>;
}
