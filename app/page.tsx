...
type Curve = {
  ...
  attitudeEulerRmse: { before: number; after: number; reductionPct: number }; rows: Row[];
};
...
function actualPoints(rows: Row[], axis: "roll" | "pitch" | "yaw", start: number, end: number) {
  ...
  return selected.filter((_, i) => i % stride === 0 || i === selected.length - 1).map((row) => [row.t, row[axis]]);
}
  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <aside className="panel">
        ...
        <div className="metrics"><span>优化前 Euler RMSE <b>{curve.attitudeEulerRmse.before}°</b></span><span>优化后 Euler RMSE <b className="teal">{curve.attitudeEulerRmse.after}°</b></span><span className="gain">相对降幅 <b>{curve.attitudeEulerRmse.reductionPct}%</b></span></div>
      ...
      <section className="viewer">
        ...
        <div className="chart">
          ...
          <div className="tooltip">{hover ? <><b>{hover.t.toFixed(3)} s</b><span>真值 {hover[axis][0].toFixed(2)}°</span><span>前 {hover[axis][1].toFixed(2)}° / 误差 {(hover[axis][1]-hover[axis][0]).toFixed(2)}°</span><span>后 {hover[axis][2].toFixed(2)}° / 误差 {(hover[axis][2]-hover[axis][0]).toFixed(2)}°</span></> : <>移动鼠标读取真实采样点</>}</div>
        </div>
    ...
    <section className="footnote"><b>数据口径</b><span>图中每一个点来自真实 `results.csv`。为保证渲染响应速度，全程视图仅绘制窗口内的原始点抽样；放大后自动使用更高密度的真实点，不进行曲线拟合或伪造插值。</span></section>
  </main>;
}
