...
type Curve = {
  ...
  duration: number; samplePeriodMs: number; samples: number; storedSamples: number; axisRmse: Record<Axis, Metric>;
  ...
  return <main className="shell">
    ...
    {view === "curves" ? <section className="workspace">
      <aside className="panel">
        <p className="label">独立真值曲线</p>{data.curves.map((item) => <button className={`case ${item.id === curve.id ? "active" : ""}`} key={item.id} onClick={() => { setId(item.id); setZoom(1); setCenter(.5); }}>{item.label}<small>回放 {item.samples.toLocaleString()} · 显示 {item.storedSamples.toLocaleString()} 点 · {item.duration.toFixed(3)} s · Grade {item.evidenceGrade}</small></button>)}
      ...
      <section className="viewer"><div className="viewer-head"><div><p className="label">保留采样点查看器</p><h2>{curve.dataset} · {plotMode === "geodesic" ? "姿态几何误差" : `${axis} · ${plotMode === "error" ? "最短角差" : "原始 Euler 角"}`}</h2></div><div className="legend">{names.map((name, i) => <span key={name}><i style={{ background: colors[i] }} />{plotMode !== "angle" && i === 0 ? "0° 基线" : name}</span>)}</div></div>
        <div className="chart"><svg viewBox="0 0 1000 380" preserveAspectRatio="none" onWheel={onWheel} onPointerMove={onMove} onPointerDown={(event) => { drag.current = { x: event.clientX, center }; event.currentTarget.setPointerCapture(event.pointerId); }} onPointerUp={() => { drag.current = null; }} onPointerLeave={() => { drag.current = null; setHover(null); }}>{[0, .25, .5, .75, 1].map((value) => <line key={`x-${value}`} x1={value * 1000} x2={value * 1000} y1="0" y2="380" className="grid" />)}{[0, .25, .5, .75, 1].map((value) => <line key={`y-${value}`} y1={value * 380} y2={value * 380} x1="0" x2="1000" className="grid" />)}{colors.map((color, index) => <path key={color} d={path(range.points, index, range.min, range.max, range.start, range.end, plotMode !== "geodesic")} fill="none" stroke={color} strokeWidth={index === 0 ? 1.4 : 2} vectorEffect="non-scaling-stroke" />)}{hover && hover.t >= range.start && hover.t <= range.end && <line x1={((hover.t - range.start) / (range.end - range.start)) * 1000} x2={((hover.t - range.start) / (range.end - range.start)) * 1000} y1="0" y2="380" className="crosshair" />}</svg><div className="axis-label top">{range.max.toFixed(1)}°</div><div className="axis-label bottom">{range.min.toFixed(1)}°</div><div className="tooltip">{hover && hoveredValues ? <><b>{hover.t.toFixed(3)} s</b><span>参考 {hoveredValues[0].toFixed(2)}°</span><span>原始 {hoveredValues[1].toFixed(2)}° / 差 {hoveredBeforeError?.toFixed(2)}°</span><span>校正 {hoveredValues[2].toFixed(2)}° / 差 {hoveredAfterError?.toFixed(2)}°</span></> : <>移动鼠标读取保留采样点</>}</div></div>
