#!/usr/bin/env python3
"""Standalone source-time residual-persistence monitor for v6 development.
...
class SourceTimeResidualPersistenceMonitor:
    ...
    def _contribute_quiet(self, source_timestamp_us: int) -> bool:
        ...
        if self.state is MonitorState.UNQUALIFIED:
            ...
            if self._warmup_ready:
                if nis <= self.config.quiet_nis_threshold:
                    if self._contribute_quiet(source_timestamp_us):
