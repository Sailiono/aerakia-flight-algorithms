...
class RateSensitivityToolTests(unittest.TestCase):
    def test_noise_profiles_have_explicit_semantics(self) -> None:
        ...
        self.assertEqual(study.profile_density("continuous_density", 100.0, protocol), (0.002, 0.0001))
        self.assertEqual(study.profile_density("zero_sensor_noise", 100.0, protocol), (0.0, 0.0))
        accel, gyro = study.profile_density("fixed_sample_noise", 100.0, protocol)
        self.assertAlmostEqual(accel * 10.0, 0.01)
        self.assertAlmostEqual(gyro * 10.0, 0.0003490658504)
    ...
    def test_summary_distinguishes_structural_readiness_from_rank(self) -> None:
        ...
        summary = study.summarize_analyzer(report)
